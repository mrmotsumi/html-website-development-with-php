...
<html lang="en">
  ...
  <div class="main_content" >
   ...
   $query = "select * from event e, watch_later w where e.id=w.video_id AND w. user_id='$user_id'";
   ...
   if(mysqli_num_rows($result) > 0)
{
  ...
  <source src="<?php echo  $row["url"] ?>" type="video/mp4">
	...
	<button type="button"  name="remove" class="btn btn-primary btn-xs remove" id="<?php echo  $row["id"]; ?>"><i class="fa fa-times" aria-hidden="true"></i>
</button>
	  ...
	  function Book_bus(id)
 {
  ...
  var remove_id = $(this).attr("id");
		 ...
		 swal({
			  ...
			  text: "want to remove video from Favorites!",
			  icon: "success",
			  buttons: true,
			  dangerMode: true,
			})
			.then((willDelete) => {
			  if (willDelete) {
			    $.ajax({
				method: 'GET',
				data: {'delete': true, 'delete_id' : remove_id },
				url: 'action.php',
				success: function(data) {
						swal("Proof! Video removed from playlist!!", {
										  icon: "success",
